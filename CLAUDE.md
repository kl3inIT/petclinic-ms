...
31. **Single Dockerfile template for monorepo via `ARG SERVICE`** — root `Dockerfile` parameterized; build with `docker build --build-arg SERVICE=visits-service .`. Layer order is intentional: gradle wrapper → settings + build-logic + version catalog → per-module `build.gradle.kts` → pre-fetch deps (cache hit ~99%) → src copies (only this invalidates on code change). Uses `jdeps --print-module-deps` on the fat JAR to compute exact JDK module set, then `jlink` produces a ~60-80MB custom JRE — extra modules added back (`java.management.rmi`, `jdk.management.agent`, `jdk.naming.dns`) because Spring Boot Actuator + Eureka need JMX + DNS at runtime that `jdeps` reflection-blind analysis misses. Then Spring Boot layered JAR `extract` for cache-efficient runtime layers (dependencies → loader → snapshot-deps → application). **HEALTHCHECK lives in compose.yaml, not Dockerfile** — each service has different management port. Inside container, `SPRING_DOCKER_COMPOSE_ENABLED=false` else Spring Boot tries to spawn nested compose.
