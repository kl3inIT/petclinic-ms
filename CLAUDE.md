...
28. **Resilience4j `@CircuitBreaker` must wrap a PUBLIC method to be intercepted by Spring AOP** — annotation lives on a public bean method; calling that method from another method in the SAME class with plain `this.fetchPet(id)` bypasses the proxy (Spring AOP weaves the proxy outside the class). In `VisitServiceImpl`, `fetchPet`/`fetchVet`/`fetchUser` are public so the proxy intercepts via the bean reference even from inner methods. Fallback signature is `(originalArgs..., Throwable)` and returns same type. Crucially: configure `ignore-exceptions` in `config-repo/visits-service.yml` to NOT count `HttpClientErrorException$NotFound` and `BadRequestAlertException` — 404 is "healthy service, missing data" not "service down"; counting it would OPEN the circuit on legitimate not-found responses. Pair with `record-exceptions` (5xx, IOException, TimeoutException) so the CB only reacts to real health signals. Instance names must match the `name="..."` parameter on `@CircuitBreaker` (vd `customers-service`, `vets-service`, `auth-service`).

29. **SBA 4.0.0 + Thymeleaf 3.1.5 → `variables.css` SpEL restriction error** — Thymeleaf 3.1.4+ enforces a "restricted" SpEL evaluation context on CSS templates, forbidding `T(...)`, `new`, and `@bean` expressions. SBA's `variables.css` (theme injection) still uses those → runtime error `Instantiation of new objects and access to static classes or parameters is forbidden in this context`. UI degrades (default colors, log spam). Workaround: pin Thymeleaf to 3.1.3.RELEASE in `services/admin-server/build.gradle.kts` via `extra["thymeleaf.version"] = "3.1.3.RELEASE"` (BOM property override — only this module). Unpin when codecentric releases a Boot 4-compatible patch that rewrites the template without restricted-context expressions.
