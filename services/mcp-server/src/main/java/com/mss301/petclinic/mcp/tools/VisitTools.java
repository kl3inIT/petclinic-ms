...
public class VisitTools {
    ...
    public PageResult<VisitSummary> searchVisits(
            @McpToolParam(description = "Filter by pet ID. Null = any pet.", required = false)
            Long petId,
            @McpToolParam(description = "Filter by vet ID. Null = any vet.", required = false)
            Long vetId,
            @McpToolParam(description = "Filter by status: SCHEDULED|IN_PROGRESS|COMPLETED|CANCELLED.", required = false)
            String status,
    ...
    public VisitSummary getVisit(
            @McpToolParam(description = "The numeric visit ID.")
            Long visitId
    ) {
